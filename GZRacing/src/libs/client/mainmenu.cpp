...
TorcsMainMenuInit(void)
{
    ...
    GfuiTitleCreate(menuHandle, "GZRacing", 0);

    //GfuiLabelCreate(menuHandle,"Racing Car Simulator", GFUI_FONT_LARGE, 320, 420, GFUI_ALIGN_HC_VB, 0);

    GfuiMenuButtonCreate(menuHandle,"Races", "", ReSinglePlayerInit(menuHandle), GfuiScreenActivate);
    ...
    // DESABILITADO PARA CORRE‚AO DEVIDO AO SANDBOX
    //GfuiMenuButtonCreate(menuHandle, "Options & Configure", "", TorcsOptionOptionInit(menuHandle), GfuiScreenActivate);
    
    GfuiMenuDefaultKeysAdd(menuHandle);

    GfuiMenuBackQuitButtonCreate(menuHandle, "Rerturn to MacOS", "Quit GZRazing", TorcsMainExitMenuInit(menuHandle), GfuiScreenActivate);
